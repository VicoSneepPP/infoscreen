 ...
 */
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' ) && OT_PLUGIN_MODE == false ) {
  ...
  class OT_Loader {
    ...
    private function constants() {
      ...
      if ( false == OT_THEME_MODE && false == OT_CHILD_THEME_MODE ) {
        ...
        if ( true == OT_CHILD_THEME_MODE ) {
          //$path = ltrim( end( @explode( get_stylesheet_directory(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
		  $path = "option-tree"; // PV: error breaks WP in case of virtual directories. It is looking for the string "option-tree" anyway.
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        ...
        return apply_filters( 'ot_upload_text', __( 'Send to PlusPort Options', 'option-tree' ) );
